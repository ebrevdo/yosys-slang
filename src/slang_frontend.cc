...
static constexpr size_t static_chunk_cache_max_entries = 65536;
static constexpr uint64_t static_chunk_cache_max_cost = 4 * 1024 * 1024;
static constexpr uint64_t static_chunk_cache_max_width = 4096;
...
{
	RTLIL::SigSpec ret;
	for (auto [base, size, chunk] : vbits.chunk_spans()) {
		(void)base;
		(void)size;
...
static uint64_t variable_bits_cache_cost(const VariableBits &bits)
{
	...
	return bits.bitwidth() + 16 * chunks;
}
...
std::optional<RTLIL::SigSpec> EvalContext::static_rvalue_signal(ast::Expression const &expr)
{
	...
	if (netlist.static_rvalue_cache.size() < static_rvalue_cache_max_entries &&
		...
		netlist.static_rvalue_cache[&expr] = {signal, cost};
...
RTLIL::SigSpec NetlistContext::convert_static(VariableBits bits)
{
	...
	for (auto vchunk : bits.chunks()) {
		...
		case Variable::Static: {
			RTLIL::SigSpec signal;
			// Only real static chunks are cacheable. Dummy chunks below still
			// allocate placeholders, and wide slices bypass the cache cap.
			bool use_cache = vchunk.length <= static_chunk_cache_max_width;
			StaticChunkCacheKey key{vchunk.variable, vchunk.base, vchunk.length};

			if (use_cache) {
				auto cached = static_chunk_cache.find(key);
				if (cached != static_chunk_cache.end()) {
					ret.append(cached->second);
					break;
				}
			}

			signal = wire(*vchunk.variable.get_symbol())
					.extract((int)vchunk.base, (int)vchunk.length);
			if (use_cache && static_chunk_cache.size() < static_chunk_cache_max_entries &&
					static_chunk_cache_cost + vchunk.length <= static_chunk_cache_max_cost) {
				static_chunk_cache[key] = signal;
				static_chunk_cache_cost += vchunk.length;
			}
			ret.append(signal);
			break;
		}
