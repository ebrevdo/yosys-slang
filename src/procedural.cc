...
uint64_t dynamic_lhs_elements_per_update_block(uint64_t element_count, uint64_t element_width)
{
	log_assert(element_count > 0);
	log_assert(element_width > 0);

	uint64_t elements_per_block = max_dynamic_lhs_block_bits / element_width;
	if (elements_per_block == 0)
		return 1;
	return std::min(element_count, elements_per_block);
}

// Fast path for dynamic element-like writes into a static inner lvalue.
// Unsupported shapes, including initial blocks, keep the existing full-demux
// lowering path.
bool try_assign_dynamic_element_with_masking(const ast::AssignmentExpression &assign,
		...
		bool blocking)
{
	...
	uint64_t elements_per_block = dynamic_lhs_elements_per_update_block(element_count, element_width);
